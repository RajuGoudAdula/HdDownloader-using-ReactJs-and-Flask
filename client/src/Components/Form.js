...
function Form() {
  ...
  async function handledownload(url, quality, container,qualityUrl) {
    alert(`Your are started downloading video in ${quality}.${container}`);
    setDownloadProgress(1); // Reset progress bar before new download
        ...
        <div className="border rounded p-4 d-flex flex-column flex-md-row" style={{ maxWidth: '600px', margin: 'auto' }}>
          <div className="d-flex flex-column align-items-start mb-3 mb-md-0" style={styles.imageDiv}>
            ...
            <div className="d-flex flex-column">
              {filterAndRemoveDuplicates(videoDetails.data.videoQualities)
                .filter(quality => quality.hasVideo) // Filter to only include qualities with video
                .slice(0, showAll ? undefined : 3) // Limit the number of items displayed
                .map((quality, index) => (
                  <div key={index} className="my-2 d-flex justify-content-between align-items-center">
                    <h5 className="mb-0" style={{ fontWeight: "600" }}>
                      {quality.qualityLabel || 'Audio Only'} {quality.container}
                    </h5>
                    <button
                      className="btn btn-primary ml-2"
                      onClick={() => handledownload(videoDetails.data.videoLink, quality.qualityLabel, quality.container, quality.url)}
                    >
                      Download
                    </button>
                  </div>
              ...
              {filterAndRemoveDuplicates(videoDetails.data.videoQualities)
                .filter(quality => quality.hasVideo) // Ensure the count is based on filtered results
                .length > 3 && (
